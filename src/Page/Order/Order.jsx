...
const Order = () => {
  ...
  return (
    <div className="max-w-[1170px] w-full mx-auto py-14 px-4 2xl:px-20 2xl:container 2xl:mx-auto">
    ...
    <div className="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
        <div className="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
            ...
            <div className="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
                ...
                <div key={index}  className="mt-4 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full">
                    <div className="  md:pb-2 w-full md:w-40 h-60 md:h-40 ">
                        <img className="w-full h-full hidden md:block object-fill" src={item.image} alt="dress" />
                        <img className="w-full h-full md:hidden object-fill" src={item.image} alt="dress" />
                    </div>
                    <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
                        <div className="w-full flex flex-col justify-start items-start space-y-8">
                            ...
                            <div className="flex justify-start items-start flex-col space-y-2">
                                <p className="text-sm dark:text-white leading-none text-gray-800"><span className="dark:text-gray-400 text-gray-300">Category: </span> {item.category}</p>
                                <p className="text-sm dark:text-white leading-none text-gray-800"><span className="dark:text-gray-400 text-gray-300">Brand: </span> {item.brand}</p>                              
                            </div>
                        </div>
                        <div className="flex justify-between space-x-8 items-start w-full">
                            <p className="text-base dark:text-white xl:text-lg leading-6">${item.price.toFixed(2)} </p>
                            <p className="text-base dark:text-white xl:text-lg leading-6 text-gray-800">{item.cartQuantity}</p>
                            <p className="text-base dark:text-white xl:text-lg font-semibold leading-6 text-gray-800">${item.price * item.cartQuantity}</p>
                        </div>
                    </div>
                </div> )}            
            </div>
            ...
            <div className="flex justify-center flex-col md:flex-row  items-stretch w-full space-y-4 md:space-y-0 md:space-x-6 xl:space-x-8">
                <div className="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
                    ...
                    <div className="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                        <div className="flex justify-between w-full">
                            ...
                            <p className="text-base dark:text-gray-300 leading-4 text-gray-600">${cart.cartTotalAmount.toFixed(2)}</p>
                        </div>
                        <div className="flex justify-between items-center w-full">
                            ...
                            <p className="text-base dark:text-gray-300 leading-4 text-gray-600">-${(cart.cartTotalAmount.toFixed(2) * 0.05).toFixed(2)} (50%)</p>
                        </div>
                        <div className="flex justify-between items-center w-full">
                            <p className="text-base dark:text-white leading-4 text-gray-800">Shipping</p>
                            <p className="text-base dark:text-gray-300 leading-4 text-gray-600">${cart.cartTotalAmount.toFixed(2) * 0.02.toFixed(2) }</p>
                        </div>
                    </div>
                    <div className="flex justify-between items-center w-full">
                        <p className="text-base dark:text-white font-semibold leading-4 text-gray-800">Total</p>
                        <p className="text-base dark:text-gray-300 font-semibold leading-4 text-gray-600">${
                            (cart.cartTotalAmount - cart.cartTotalAmount * 0.05) + (cart.cartTotalAmount.toFixed(2) * 0.02).toFixed(2)
                        }</p>
                    </div>
                </div>
                <div className="flex flex-col justify-center px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
                    ...
                    <div className="flex justify-between items-start w-full">
                        ...
                        <p className="text-lg font-semibold leading-6 dark:text-white text-gray-800">${cart?.cartTotalAmount.toFixed(2) * 0.02 }</p>
                    </div>
